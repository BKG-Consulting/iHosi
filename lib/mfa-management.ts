...
export const HIPAA_MFA_CONFIG: MFAConfig = {
  roles: {
...
export class MFAManager {
   ...
   */
  static async checkMFARequirement(userId: string, userRole: string): Promise<{
    required: boolean;
    enforced: boolean;
    ...
    try {
      ...
      const requirement = HIPAA_MFA_CONFIG.roles[userRole] || MFARequirement.NONE;
      ...
      if (requirement === MFARequirement.REQUIRED && !hasValidMFA) {
        ...
        gracePeriodEnds = new Date(userCreated.getTime() + HIPAA_MFA_CONFIG.gracePeriodDays * 24 * 60 * 60 * 1000);
      }
      ...
      return {
        ...
        gracePeriodEnds,
        methods: HIPAA_MFA_CONFIG.allowedMethods,
        hasValidMFA
      };
   ...
   */
  static async enforceMFARequirements(): Promise<{
    allowed: boolean;
    redirectTo?: string;
    reason?: string;
  }> {
   ...
   */
  static async generateBackupCodes(userId: string): Promise<{
    success: boolean;
    codes?: string[];
    error?: string;
  }> {
    ...
    try {
      ...
      for (let i = 0; i < HIPAA_MFA_CONFIG.backupCodeCount; i++) {
   ...
   */
  static async validateMFASetup(userId: string): Promise<{
    valid: boolean;
    issues: string[];
    ...
    try {
      ...
      if (!user.twoFactorEnabled) {
        const requirement = HIPAA_MFA_CONFIG.roles[userRole];
   ...
   */
  static async generateMFAComplianceReport(): Promise<{
    totalUsers: number;
    mfaEnabled: number;
    complianceByRole: Record<string, { total: number; compliant: number; percentage: number }>;
    nonCompliantUsers: Array<{ userId: string; role: string; gracePeriodEnds?: Date }>;
  }> {
    ...
    try {
      ...
      const complianceByRole: Record<string, { total: number; compliant: number; percentage: number }> = {};
      const nonCompliantUsers: Array<{ userId: string; role: string; gracePeriodEnds?: Date }> = [];
   ...
   */
  static async sendMFAReminders(): Promise<{
