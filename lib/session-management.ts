...
export class SessionManager {
  ...
  static async createSession(userId: string, userRole: string): Promise<string> {
    try {
      ...
      if (!DEFAULT_SESSION_CONFIG.allowConcurrentSessions) {
      ...
      if (activeSessions >= DEFAULT_SESSION_CONFIG.maxConcurrentSessions) {
        throw new Error('Maximum concurrent sessions exceeded');
      }

      // Create session token
      const sessionToken = this.generateSessionToken();
      const expiresAt = new Date(Date.now() + DEFAULT_SESSION_CONFIG.timeoutMinutes * 60 * 1000);
      ...
      await logAudit({
        ...
        metadata: {
          ...
          ipAddress,
          userAgent,
          sessionDuration: DEFAULT_SESSION_CONFIG.timeoutMinutes
        }
      });
  ...
  static async updateActivity(sessionToken: string): Promise<boolean> {
    try {
      ...
      const newExpiresAt = new Date(Date.now() + DEFAULT_SESSION_CONFIG.timeoutMinutes * 60 * 1000);
   ...
   */
  static async terminateSession(sessionToken: string, reason: string): Promise<boolean> {
    try {
      const session = await db.userSession.findUnique({
        where: { session_token: sessionToken }
      });
   ...
   */
  static async terminateExistingSessions(userId: string, reason: string): Promise<number> {
    try {
  ...
  static async recordLoginAttempt(
    ...
    try {
      ...
      const existingLockout = await db.userLockout.findFirst({
        where: {
          OR: [
            { email },
            { user_id: userId }
      ...
      const attemptsRemaining = DEFAULT_SESSION_CONFIG.maxFailedAttempts - recentAttempts;

      // Check if lockout threshold reached
      if (recentAttempts >= DEFAULT_SESSION_CONFIG.maxFailedAttempts) {
        const unlockAt = new Date(Date.now() + DEFAULT_SESSION_CONFIG.lockoutDurationMinutes * 60 * 1000);
        ...
        await logAudit({
          ...
          metadata: {
            ...
            ipAddress,
            lockoutDuration: DEFAULT_SESSION_CONFIG.lockoutDurationMinutes
          }
        });
   ...
   */
  static async checkLockout(email: string, userId?: string): Promise<{
    locked: boolean;
    unlockAt?: Date;
    reason?: string;
  }> {
  ...
  static async cleanupExpired(): Promise<void> {
    try {
      ...
      const expiredLockouts = await db.userLockout.deleteMany({
        where: {
          unlock_at: { lt: now }
        }
      });
  ...
  private static async getActiveSessionCount(userId: string): Promise<number> {
    return await db.userSession.count({
      where: {
        user_id: userId,
        is_active: true,
...
export async function validateUserSession(): Promise<{
  ...
  try {
    ...
    const lockoutCheck = await SessionManager.checkLockout(
