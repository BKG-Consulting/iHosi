...
interface EnhancedPatientBookingProps {
  patientId: string;
  className?: string;
}
...
export default function EnhancedPatientBooking({ patientId, className }: EnhancedPatientBookingProps) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const transformedDoctors = data.doctors?.map((doctor: any) => ({
          ...
          aiRecommended: Math.random() > 0.5, // Random AI recommendation
  ...
  return (
    <div className={`max-w-4xl mx-auto space-y-6 ${className}`}>
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-gray-600">Simple, fast, and AI-powered scheduling</p>
        {aiEnabled && (
          ...
          <CardContent className="space-y-4">
            {doctors.map((doctor) => (
              <div
                ...
                className="p-4 border rounded-xl cursor-pointer hover:shadow-lg transition-all duration-200 bg-white hover:bg-blue-50"
              >
